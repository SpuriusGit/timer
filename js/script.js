const deadline = "2023-12-8";

function getTimeRemaining(endtime) {
  let now = new Date();
  const t = Math.floor(
      (Date.parse(deadline) - Date.parse(now)) / (1000 * 60 * 60 * 24)
    ),
    days = Math.floor(
      (Date.parse(deadline) - Date.parse(now)) / (1000 * 60 * 60 * 24)
    ),
    ...
    minutes = Math.floor(
      ((Date.parse(deadline) - Date.parse(now)) / (1000 * 60)) % 60
    ),
    seconds = Math.floor(
      ((Date.parse(deadline) - Date.parse(now)) / 1000) % 60
    );
  return {
    total: t,
    days: days,
